// Map hex color từ AssetStatus trong DB sang Tailwind classes
const HEX_TO_THEME: Record<string, 'green' | 'blue' | 'orange' | 'red' | 'gray'> = {
  '#10b981': 'green',  // IN_USE  - Đang sử dụng
  '#3b82f6': 'blue',   // READY   - Sẵn sàng cấp phát
  '#f59e0b': 'orange', // MAINTENANCE - Đang bảo trì
  '#ef4444': 'red',    // BROKEN  - Hỏng
  '#9ca3af': 'gray',   // RETIRED - Đã thanh lý
};

type ThemeColor = 'green' | 'blue' | 'orange' | 'red' | 'gray';

function resolveTheme(color?: string): ThemeColor {
  if (!color) return 'gray';
  return HEX_TO_THEME[color.toLowerCase()] ?? 'gray';
}

export const getStatusBadgeClasses = (color?: string) => {
  switch (resolveTheme(color)) {
    case 'green':
