...
import { useDeletePerson } from '../../hooks/useMasterData';
...
export function DeletePersonModal({ isOpen, onClose, person }: Props) {
  ...
  const handleDelete = async () => {
    try {
      ...
      setError(err.response?.data?.message || err.response?.data?.error?.message || 'Có lỗi xảy ra khi xóa nhân viên');
    }
  };
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md overflow-hidden border border-gray-200 dark:border-gray-800">
        ...
        <div className="p-6">
          <p className="text-gray-600 dark:text-gray-300 text-sm mb-4">
            Bạn có chắc chắn muốn xóa nhân sự <strong className="text-gray-900 dark:text-white">{person.fullName}</strong> ({person.employeeCode}) không?
          </p>
          ...
          <div className="p-4 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 text-sm rounded-lg border border-amber-200 dark:border-amber-900/50">
            ...
            <ul className="list-disc pl-5 mt-1 space-y-1">
              <li>Nhân sự không thể bị xóa nếu vẫn còn đang giữ thiết bị.</li>
              <li>Nếu nhân sự này có liên kết với tài khoản đăng nhập (User), tài khoản đó cũng sẽ bị vô hiệu hóa.</li>
            </ul>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 flex justify-end gap-3">
          ...
          >
            {deleteMutation.isPending ? 'Đang xử lý...' : 'Xóa nhân sự'}
          </button>
        </div>
      </div>
    </div>
