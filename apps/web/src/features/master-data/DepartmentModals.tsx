...
import { useCreateDepartment, useUpdateDepartment, useDeleteDepartment } from '../../hooks/useMasterData';

export function DepartmentFormModal({ isOpen, onClose, entity }: { isOpen: boolean, onClose: () => void, entity?: any }) {
  ...
  useEffect(() => {
    if (entity) setFormData({ code: entity.code, name: entity.name });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 rounded-xl w-full max-w-lg p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{entity ? 'Sửa' : 'Thêm'} Phòng ban</h2>
...
export function DepartmentDeleteModal({ isOpen, onClose, entity }: { isOpen: boolean, onClose: () => void, entity: any }) {
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 rounded-xl p-6 w-full max-w-sm">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Xóa Phòng ban</h2>
