...
import { useCreateLocation, useUpdateLocation, useDeleteLocation } from '../../hooks/useMasterData';

export function LocationFormModal({ isOpen, onClose, entity }: { isOpen: boolean, onClose: () => void, entity?: any }) {
  ...
  useEffect(() => {
    if (entity) setFormData({ code: entity.code, name: entity.name, type: entity.type || 'Office' });
    else setFormData({ code: '', name: '', type: 'Office' });
  }, [entity, isOpen]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 rounded-xl w-full max-w-lg p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{entity ? 'Sửa' : 'Thêm'} Vị trí</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input required value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full border p-2 rounded bg-white dark:bg-gray-800" />
          </div>
...
export function LocationDeleteModal({ isOpen, onClose, entity }: { isOpen: boolean, onClose: () => void, entity: any }) {
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 rounded-xl p-6 w-full max-w-sm">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Xóa Vị trí</h2>
